...
try:
    ...
    cursor.execute("SELECT DISTINCT wines.name, sales.quantity, distributer.name from sales "
                   "join distributer on distributer.distributer_id = sales.distributer_id "
                   "join wines on wines.wine_id = sales.wine_id ")

    rows = cursor.fetchall()

    fieldNameArray = ['Wine Name', 'Sales Quantity', 'Results', 'Distributer(s) Name']
    now = datetime.now()
